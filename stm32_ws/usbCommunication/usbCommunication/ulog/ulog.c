        ...
        const char *format, va_list args)
{
    ...
    /* add time info */
    {
        snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len, "[%ld.%03ld]", ulog_get_time_ms()/1000, ulog_get_time_ms()%1000);
...
#define __is_print(ch)       ((unsigned int)((ch) - ' ') < 127u - ' ')
    ...
    for (i = 0, log_len = 0; i < size; i += width)
    {
        ...
        fmt_result = snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE, "%04lX-%04lX: ", i, i + width);
        /* calculate log length */
        if ((fmt_result > -1) && (fmt_result <= ULOG_LINE_BUF_SIZE))
        {
            log_len += fmt_result;
