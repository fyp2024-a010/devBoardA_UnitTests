...
local_info_t protocol_local_info = {.is_valid = 0};
 ...
 * @param  address  Protocol local address, which cannot be changed after
 * initialization. Each device in the same network occupies a unique address.
...
/**
 * @brief Flushes the send list, calling this function will send the data in the
 * send list. Call after calling protocol_send or protocol_ack, or call
 * periodically.
 * @param void
 * @retval Protocol return status
 */
 ...
 * @retval Protocol return status
 */
uint32_t protocol_rcv_data(void *p_data, uint32_t data_len,
